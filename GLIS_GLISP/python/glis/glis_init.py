def init(prob):
    ...
    if not("isUnknownFeasibilityConstrained" in prob):
        prob["isUnknownFeasibilityConstrained"] = False

    if not("isUnknownSatisfactionConstrained" in prob):
        prob["isUnknownSatisfactionConstrained"] = False

    if not("g_unkn_fun" in prob):
        prob["g_unkn_fun"] = []
    
    if not("s_unkn_fun" in prob):
        prob["s_unkn_fun"] = []
