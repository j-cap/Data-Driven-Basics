...
predictor = PARC(K=K, alpha=alpha, sigma=sigma, separation=separation, maxiter=maxiter,
                 cost_tol=1e-4, min_number=int(len(Y_train) / 100),
                 beta=beta, fit_on_partition=True, softmax_maxiter=softmax_maxiter,
                 verbose=1)
...
print("\nRegression results on dataset '%s':\n" % dataset)
...
print("\nRegression results on dataset (reduced partition) '%s':\n" % dataset)
print("Training data: %6.2f %%" % (score_train_subset * 100))
print("Test data:     %6.2f %%" % (score_test_subset * 100))
print("--------------------\n")
...
print("\nClassification results on dataset '%s':\n" % dataset)
print("Training data: %6.2f %%" % (score_train * 100))
print("Test data:     %6.2f %%" % (score_test * 100))
print("--------------------\n")
